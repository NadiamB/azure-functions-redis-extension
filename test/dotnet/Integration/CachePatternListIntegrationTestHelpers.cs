...
{
    internal static class CachePatternListIntegrationTestHelpers
    {
        internal static Process StartFunction(string functionName, int port)
        {
            ...
            if (!hostStarted.Task.Wait(TimeSpan.FromMinutes(3)))
            {
                throw new Exception("Azure Functions Host did not start");
            }
            if (!functionLoaded.Task.Wait(TimeSpan.FromMinutes(3)))
            {
